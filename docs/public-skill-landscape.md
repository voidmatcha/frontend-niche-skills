...
| Pointer and gesture | mgifford `touch-pointer` is the accessibility complement. A previously listed direct missing-release match (lennondotw `pointer-drag-release`) is no longer publicly available (404 on 2026-09-24). | Keep `pointer-gesture-contracts` only for its broader single-pointer event-delivery, `touch-action`, teardown, and sibling-routing contract. |
