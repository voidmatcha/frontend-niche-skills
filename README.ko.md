...
패턴의 가치: 또 하나의 범용 패치를 적용하기 전에 에이전트가 올바른 가설부터 검증하게 만듭니다.
...
5. **올바른 호스트에서 검증합니다.** WebView 버그에는 앱 WebView 증거가, 결제 페이지 발견에는 런타임 스크립트/PAN 경계 증거가, 시각적 충실도에는 레퍼런스/렌더 캡처가 필요하며, 폼/접근성 발견은 가능하면 회귀 테스트로 남기세요.

## Skills

31개의 skill을, 각 skill이 겨냥하는 프론트엔드 실패 유형별로 묶었습니다. 리포트가 어수선하거나 여러 도메인에 걸쳐 있다면 `frontend-report-triage`부터 시작하세요.

실용적인 우선순위:

- **기본 고가치 검사:** SSR/딥링크 라우팅, 폼 검증, 날짜/시간, 인증/보안, 결제/내보내기 경계, 오버레이, 접근성, 시맨틱 HTML. 흔하거나 릴리스 후 비용이 큰 버그를 잡아냅니다.
- **호스트/제품 특화 검사:** WebView, CJK/IME, i18n/RTL, 결제 페이지 증거는 제품이 실제로 그 표면을 출시할 때 가장 가치가 큽니다.
- **품질/유지보수 검사:** 디자인 충실도와 컴포넌트 추출은 리뷰의 고통이 런타임 버그가 아니라 시각적 드리프트, AI가 생성한 UI, 섣부른 추상화일 때 유용합니다.

출처 모델: README는 라우팅 문서와 증거 문서를 나열하고, 상세한 인용은 각 skill의 `## Sources` 블록 또는 `references/*.md` 파일에 둡니다. README가 모든 upstream URL을 중복해서 담지 않기 위해서입니다.

### 여기서 시작

| Skill | 사용 시점 |
| --- | --- |
| [`frontend-report-triage`](./skills/frontend-report-triage/SKILL.md) | 모호하거나 증상이 여럿인 프론트엔드 버그 리포트를 팩 전체에 걸쳐 트리아지합니다. 가능성 높은 실패 클래스, 증거 공백, 가장 적합한 후속 skill 1-3개를 돌려줍니다. |

### 런타임 호스트 엣지

| Skill | 사용 시점 |
| --- | --- |
| [`webview-bridge-pages`](./skills/webview-bridge-pages/SKILL.md) | 네이티브 WebView 안에서 로드되는 페이지를 만들거나 디버깅할 때: bridge 계약, safe-area/뷰포트 레이아웃, 생명주기, 히트 테스트 vs 페인트/컴포지팅, 앱 호스트 특이 동작. |
...
| [`realtime-transport-contracts`](./skills/realtime-transport-contracts/SKILL.md) | 연결 끊김을 넘나드는 WebSocket/SSE 클라이언트를 디버깅할 때: 재연결 backoff/jitter, SSE Last-Event-ID/커서 재개, 순서 뒤바뀜/중복/공백 델타, heartbeat/좀비 감지, bufferedAmount 배압, 열린 소켓에서의 인증 갱신. |
...
| [`view-transitions-contracts`](./skills/view-transitions-contracts/SKILL.md) | 조용히 중단되거나, 오래된 snapshot에서 멈추거나, reduced-motion을 무시하거나, 고스트를 남기는 View Transitions API 애니메이션을 리뷰할 때 — 재구현 가이드가 아니라 리뷰/PR 가치 판단의 관점입니다. |
...
| [`responsive-image-contracts`](./skills/responsive-image-contracts/SKILL.md) | 반응형 이미지 마크업을 리뷰할 때: 실제 레이아웃 대비 `srcset`/`sizes`, 고유 너비(intrinsic-width) 표기, LCP eager/`fetchpriority`, `picture` 아트 디렉션, CLS를 위한 `width`/`height`. |
...
| [`js-form-validation-contracts`](./skills/js-form-validation-contracts/SKILL.md) | React Hook Form, Formik, Final Form, vee-validate, Valibot 또는 커스텀 JS 폼 플로우: 오래된(stale) 오류, 비활성화된 제출 버튼, 비동기/서버 경쟁, 서버 필드 오류 매핑. |
...
| [`optimistic-update-rollback-contracts`](./skills/optimistic-update-rollback-contracts/SKILL.md) | 낙관적 UI 뮤테이션: 서버 확인 전에 변경 적용, 임시 ID vs 서버 ID, 실패 시 롤백, refetch/무효화와의 조정(reconcile), 응답과 백그라운드 refetch 사이의 경쟁. |
...
| Skill | 사용 시점 |
| --- | --- |
| [`core-web-vitals-performance-contracts`](./skills/core-web-vitals-performance-contracts/SKILL.md) | 실패하는 Core Web Vital(LCP, CLS, INP, TTFB)을 고치기 전에 특정 요소, 레이아웃 이동, 메인 스레드 작업으로 귀속시킬 때 — 점수만이 아니라 페이지 전체의 예산 관리입니다. |
...
| [`async-effect-race-contracts`](./skills/async-effect-race-contracts/SKILL.md) | 날것의 비동기 effect가 오동작할 때: 의존성 변경 시 fetch 경쟁(오래된 응답이 이김), 누락된 cleanup/`AbortController`, StrictMode 이중 호출, interval/구독의 오래된 클로저. |
| [`pwa-offline-cache-contracts`](./skills/pwa-offline-cache-contracts/SKILL.md) | service worker/오프라인 캐싱이 잘못될 때: 배포 후 오래된 빌드, `ChunkLoadError`, precache 공백, 캐시 버전 관리/축출, SW 업데이트 생명주기, 인증된 응답 캐싱. |
...
| WebSocket 또는 SSE 클라이언트가 연결 끊김을 넘기지 못합니다: 재연결 폭풍, 중복/누락 이벤트, 순서가 뒤바뀐 델타, OPEN이지만 죽은 소켓에서 멈춘 UI, 버퍼 증가, 핸드셰이크 후 만료된 토큰. | `realtime-transport-contracts` | 재연결/backoff, 재개/커서, 델타 접기, 생존/heartbeat, 배압, 소켓 재인증 중 무엇의 문제인가? |
...
| 돈/수량 합계가 소수점이나 한 푼 단위로 어긋나거나, 반올림이 이상해 보이거나, 현지화된 금액이 잘못 파싱됩니다. | `money-and-precision-contracts` | 값이 이진 부동소수점으로 계산되는가, 그리고 계산이 어떤 반올림 모드와 최소 단위 표현을 쓰는가? |
...
| 페이지로 들여온 파일이 오동작합니다: 드롭 존 하이라이트가 깜빡이거나, 드롭한 파일이 페이지를 다른 곳으로 이동시키거나, 드롭한 폴더에서 아무것도 나오지 않거나, 잘못된 타입의 파일이 통과하거나, 이미지 붙여넣기가 깨지거나, 미리보기 URL이 누수됩니다. | `file-ingest-contracts` | 어느 수집 고리가 실패하는가: drag 이벤트 취소, `DataTransfer` items vs files, 타입 신뢰, 붙여넣기, object URL 생명주기? |
...
| View Transitions 애니메이션이 오동작합니다: 무작위로 실행되지 않거나(조용한 중단), 오래된 프레임에서 멈추거나, reduced-motion을 무시하거나, 고스트 morph를 남깁니다. | `view-transitions-contracts` | 중복된 `view-transition-name`인가, 페인트되지 않은 snapshot(Suspense/decode)인가, 누락된 reduced-motion 블록인가, 잘못된 Transition 래핑인가? |
...
| 페이지의 LCP/CLS/INP가 실패하고, 점수 보고가 아니라 원인 귀속이 필요합니다. | `core-web-vitals-performance-contracts` | LCP 요소는 무엇이고(발견 가능한가/우선순위가 잡혔는가), 각 레이아웃 이동의 출처는 무엇이며, 어떤 long task가 INP를 부풀리는가? |
...
| 리포트가 모호하거나, 여러 도메인에 걸쳐 있거나, 어떤 전문 skill이 맡아야 할지 확신이 없습니다. | `frontend-report-triage` | 가능성 높은 실패 클래스 상위 1-3개는 무엇이고, 이를 구별해 줄 증거는 무엇인가? |
...
아니요. 이 skill들은 범용 UI 리뷰가 자주 놓치는 프론트엔드 엣지에 집중합니다: WebView 호스트 동작, 네이티브 HTML 구조, IME/CJK 입력, 접근성 계약, 하이드레이션, 폼, 날짜/시간, 인증, 결제 페이지 클라이언트 증거, 내보내기, 오버레이, 디자인 충실도.
