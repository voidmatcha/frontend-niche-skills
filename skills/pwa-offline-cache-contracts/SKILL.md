...
description: "Use when a service worker or offline cache serves the wrong bytes: users stay on a stale build after a deploy because the new worker sits in 'waiting' and never activates; a long-cached index.html references content-hashed chunks the deploy purged, so navigation throws ChunkLoadError or 'Failed to fetch dynamically imported module'; custom Workbox globPatterns replaced the defaults so fonts/icons/routes are missing offline; old versioned caches are never deleted in activate and pile up; one 404 makes cache.addAll reject so the whole install aborts and nothing caches; skipWaiting/clients.claim ships silently with no prompt-to-reload; there is no navigation fallback or offline page; or an authenticated HTML/API response lands in Cache Storage and is served to another session. Service-worker and Cache Storage asset-caching lifecycle scope: for capturing the SW error / ChunkLoadError in monitoring see client-error-observability-contracts, for the runtime data-fetch cache (SWR/TanStack/Apollo) vs the SW asset cache see frontend-data-fetching-cache-contracts, for the policy on caching authenticated responses see frontend-security-baseline."
---
...
Treat hits as leads; open the SW source and the build config before filing. Route mechanical checks to Lighthouse's PWA/installability + offline audit and your build tool's Workbox output (glob count, precache size, size-limit warnings); DevTools Application > Service Workers shows the waiting worker and the Update-on-reload switch.
