...
5. **Check clipboard permission fallback** — `navigator.clipboard` usually needs a secure context and user activation. Catch failures, provide visible feedback or a legacy fallback where project policy requires it, and avoid silent success UI.
...
| Clipboard write errors ignored while UI says copied | Safari, permissions, or missing user activation can turn into false success. | Catch rejection, show failure/fallback, and test the rejected promise path. |
...
- Clipboard API use is not wrong by itself; the issue is unhandled rejection, missing user activation, or false success UI.
