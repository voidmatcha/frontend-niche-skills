...
description: "Use when client-side data reads become stale or wasteful against a cache: mutations invalidate the wrong key/tag, unstable or incomplete keys cause misses/collisions, requests form avoidable waterfalls, focus/mount creates refetch storms, stale settings prevent refresh, pagination replaces pages or grows without bounds, or an Apollo fetch policy never reaches the network. Covers TanStack Query, SWR, RTK Query, Apollo, and hand-rolled read caches. Use optimistic-update-rollback-contracts for write/rollback behavior, realtime-transport-contracts for the pushed-delta transport (reconnect, resume, ordering), ssr-hydration-mismatch for initial cache hydration, and async-effect-race-contracts for raw fetch-in-effect races."
---
...
7. **Apollo fetchPolicy/nextFetchPolicy: do not strand a query off-network or keep it on the network by accident.** `cache-only`/`cache-first` can leave a screen showing empty or stale data that never revalidates; `cache-and-network`/`network-only` keeps going back to the network on later executions and cache updates unless `nextFetchPolicy` demotes it to `cache-first` (`fetchPolicy` governs the first execution, `nextFetchPolicy` how the query responds to future cache updates). `nextFetchPolicy` resets to the initial policy when variables change (reason `variables-changed`).
...
- **realtime-transport-contracts** — the live transport itself (WebSocket/SSE reconnect, resume cursor, delta dedupe/order/gap folding, liveness). How a received update is written into the client query cache — which key it targets, and `setQueryData` versus invalidate — stays in this skill, including healthy deltas that never write to or invalidate the rendered query key.
...
- Apollo Client — Queries (`fetchPolicy` "is used for the query's first execution" and `nextFetchPolicy` "is used to determine how the query responds to future cache updates"; reset on variables change): <https://www.apollographql.com/docs/react/data/queries>
