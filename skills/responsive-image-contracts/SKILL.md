...
description: "Use when reviewing responsive image markup and the wrong file ships: a w-descriptor srcset with no sizes (browser assumes 100vw and over-downloads the widest candidate), a sizes value that lies about the layout (100vw on a 3-column grid), intrinsic-width descriptors that do not match the file (a 1200px file tagged 640w), a hero/LCP image left loading=lazy or missing fetchpriority=high, an <img> without width/height or aspect-ratio causing CLS, too few srcset candidates, x (density) vs w (width) descriptor misuse, a <picture> art-direction that omits media/type on its <source>, or sizes=auto without loading=lazy. Review-judgment scope: for rendered-vs-design visual/pixel drift see design-to-code-fidelity, for localized caption/filename/alt copy see i18n-copy-and-layout; owns image-load priority for the LCP element but not whole-page Core Web Vitals budgeting."
---
