...
9. **`sizes="auto"` is only valid with `loading="lazy"`.** On an eager image the HTML sizes parser ignores the `auto` entry, so a bare `sizes="auto"` falls through to the spec's `100vw` default (over-download). Keep explicit `sizes` lengths for eager/hero images; on lazy ones, prefer `auto` followed by an explicit list (`sizes="auto, (max-width: 30em) 100vw, 50vw"`, the spec's own fallback form for user agents without `auto`), and keep `width`/`height` for the aspect ratio.
...
- Web Platform features, [`<img sizes="auto">`](https://web-platform-dx.github.io/web-features-explorer/features/sizes-auto/) — `sizes="auto"` applies only to `loading="lazy"` images.
- WHATWG HTML, [parse a sizes attribute](https://html.spec.whatwg.org/multipage/images.html#sizes-attributes) ("If size is auto, and img is not null, and img is being rendered, and img allows auto-sizes, then set size to the concrete object size width of img"; "If size is still auto, then it will be ignored"; the algorithm ends "Return 100vw") and its backwards-compatible example `sizes="auto, (max-width: 30em) 100vw, (max-width: 50em) 50vw, calc(33vw - 100px)"`.
