...
- Use **payment-page-client-security** when the question is PAN/CVV visibility, the payment page's script inventory and CSP/SRI evidence, or PCI evidence. A hosted card-field iframe's host/guest contract — `sandbox` tokens, `allow` delegation, READY/`postMessage` — stays in this skill; payment resumes the PAN-boundary check once the frame renders.
- Use **user-activation-contracts** for other gesture-gated calls (popups, clipboard, fullscreen) that fail after async work. `document.requestStorageAccess()` stays in this skill: its gesture timing and activation consumption (item 7), the embed-side session exchange, and the denial/recovery path.
...
This repository includes a live regression fixture at [`tests/iframe-contract.bats`](./tests/iframe-contract.bats). It starts parent and guest servers on distinct loopback ports and, when the Playwright CLI plus its Chromium browser are installed, checks the authenticated replayable handshake, wrong-source/origin rejection, bounded grow-and-shrink sizing, listener teardown, accessible title, and a `frame-ancestors` rejection from a disallowed parent origin. The Bats test skips cleanly when the Playwright CLI is unavailable; source/audit checks still run.
...
- MDN, `Document.requestStorageAccess()` — requests "are automatically denied unless the embedded content is currently processing a user gesture such as a tap or click (transient activation), or unless permission was already granted previously"; "If the promise resolves (i.e., if permission was granted), then the user gesture has not been consumed … If the promise rejects (i.e., permission was not granted), then the user gesture has been consumed, so the script can't do anything that requires a gesture" (checked 2026-09-25): <https://developer.mozilla.org/en-US/docs/Web/API/Document/requestStorageAccess>
- CSSOM View, `scrollHeight` — for the root element (no quirks mode) it returns "max( viewport scrolling area height, viewport height )", so it never reports less than the iframe's current height: <https://drafts.csswg.org/cssom-view/#dom-element-scrollheight>
