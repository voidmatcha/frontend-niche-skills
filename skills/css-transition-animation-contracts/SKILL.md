...
4. **`::backdrop` needs its own top-level rule and its own `@starting-style`.** Nesting selectors cannot express a pseudo-element, so `dialog::backdrop` / `[popover]::backdrop` must be a separate rule with its own `display`/`overlay`/`allow-discrete` transition and, if it animates in, its own `@starting-style`. A backdrop folded into the element's nested block does not fade.
...
6. **Gate on the animation finishing, not on one event.** Prefer `Promise.all(el.getAnimations({subtree:true}).map(a => a.finished))` — one code path that settles on complete or cancel. Mind that `finished` *rejects* (`AbortError`) when an animation is cancelled: run cleanup via `.then(done, done)` / `finally`, never bare `.then(done)`, or the cancel path skips cleanup and surfaces an unhandled rejection. If you must use events, listen to `transitionrun` + `transitionend` + `transitioncancel` together and add a duration-based timeout fallback so a missing event cannot wedge the lifecycle.
...
4. The fix is narrow: add `display`+`overlay` to one transition list, move/split one `transition-behavior` line, re-target one `@starting-style`, split out one `::backdrop` rule, or swap one `transitionend` gate for `getAnimations().finished` plus a timeout.
