...
3. **`@starting-style` must match the open state and sit after the rule it starts from.** Transitions do not fire on first style update or on the `display:none`->visible flip, so the entry animation needs a `@starting-style` block defining the from-state. It must win the cascade against the rule that sets the open-state values: select the *open* state (`:popover-open`, or `[open]` for `<dialog>`) with at least that rule's specificity and be written after it (MDN: "If you specify the `@starting-style` at-rule before the 'original rule', the original styles will override the starting styles"). A starting style that loses — a base/closed selector against a later or more specific `[open]`/`:popover-open` rule, or any selector placed before an equal-specificity original rule — is overridden and the entry silently does not animate.
