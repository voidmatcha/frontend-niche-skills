...
| `scripts/render-capture.mjs <url> <out.png> [w] [h] [scale]` | Capture a web/Storybook/local-route screenshot at the target viewport and device scale. Supports `INIT_SCRIPT`, `NEUTRALIZE_CSS`, and `ALLOW_ANIMATION=1` (keeps CSS animations/transitions running; JS-driven motion via WAAPI/rAF/canvas/GSAP is unaffected — neutralize it in `INIT_SCRIPT`). | `@playwright/test` or `playwright` resolvable from cwd |
