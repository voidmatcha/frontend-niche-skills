...
description: "Use when raw async effects race or leak below a data library: an older fetch result overwrites a newer one, cleanup is missing after unmount, StrictMode exposes non-idempotent setup, a long-lived callback reads a stale closure, or an effect loops because its dependencies are unstable. Covers take-latest guards, AbortController, mirrored cleanup, and dependency discipline. Use frontend-data-fetching-cache-contracts when a data library owns the read, optimistic-update-rollback-contracts for mutation rollback races, and realtime-transport-contracts for socket recovery."
---
...
2. **Every setup that subscribes/opens/schedules needs a cleanup that undoes it** — same function reference for `removeEventListener`. React 18 removed the "state update on an unmounted component" warning, so a missing return now leaks silently — the console will not flag it for you.
