...
2. **Every setup that subscribes/opens/schedules needs a cleanup that undoes it** — `subscribe`→`unsubscribe`, `addEventListener`→`removeEventListener` (same function reference), `setInterval`/`setTimeout`→`clear*`, `connect`→`disconnect`. A missing return accumulates a new listener/interval/connection on every re-run — that is the real leak; a late async `setState` after unmount is harmless by itself, but React 18 removed its warning, so the console no longer flags either case.
