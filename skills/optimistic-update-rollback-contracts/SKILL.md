...
description: "Use when a UI applies a mutation before the server confirms and the reconciliation goes wrong — an optimistic change that never rolls back after the request fails, a created row that shows up twice (the temp entry plus the refetched real one), an item that flickers or reverts then reappears when the server response and a background refetch race, a temp client id that is never swapped for the server id (so a later edit/delete hits the wrong record or leaves an orphaned temp row), concurrent mutations to the same resource landing out of order, or a `useOptimistic`/`optimisticData` value that just snaps back with nothing persisted. Optimistic-apply / rollback / reconcile lifecycle scope; for submit and double-submit async races inside a form see js-form-validation-contracts, for announcing success or rollback to assistive tech see a11y-contract-testing, for server-vs-client initial-render divergence see ssr-hydration-mismatch."
---
...
3. **Wire rollback to every failure exit.** `onError` restores the snapshot; a `mutationFn`/promise that rejects (or throws before the request even leaves) with no rollback leaves the fake state permanently. SWR rolls back by default (`rollbackOnError`), but confirm the error types you exempt (e.g. `AbortError`) are exempted on purpose.
