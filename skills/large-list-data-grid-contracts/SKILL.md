...
description: "Use when reviewing or building a virtualized list or data grid and behavior breaks at the seams of windowing: scroll jumps or flickers on variable-height rows because estimateSize is wrong and measureElement corrections shift offsets (worst when prepending items in a chat or feed), overscan too small so rows blank white on fast scroll or too large so INP and memory suffer, a test that disables virtualization (disableVirtualization, onShouldVirtualize false, renderAllRows) staying green while production breaks, features that assume rows are all in the DOM silently failing — Ctrl+F and find-in-page match nothing off-screen, a screen reader announcing '3 of 20' (the mounted rows) instead of '3 of 5000' because aria-setsize/aria-posinset (or aria-rowcount/aria-rowindex for grids) are missing, focus lost when the focused row unmounts on scroll, or a sticky header and pinned column drifting out of alignment with the scrolling body. Virtualization/windowing contract scope; route the row/set-size aria semantics as a test contract to a11y-contract-testing, focus restore when rows unmount to overlay-focus-scroll-contracts, images inside virtualized rows to responsive-image-contracts, and the paginated/infinite query feeding the list to frontend-data-fetching-cache-contracts."
---
