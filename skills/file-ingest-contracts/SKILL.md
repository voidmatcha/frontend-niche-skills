...
description: "Use when bringing files INTO the page and the ingest path misbehaves: a drop-zone highlight that flickers as the pointer crosses child elements, a dropped file that opens in the browser or navigates the page away instead of uploading, a drop that never fires, a dropped folder that yields no files, files that pass the picker but are the wrong true type (accept attribute or file.type trusted over magic bytes), broken paste-image-from-clipboard, or preview thumbnails that leak memory because object URLs are never revoked. Covers drag-and-drop (dragover preventDefault, dropEffect, dragenter/leave counter), DataTransfer.items vs files, directory/multiple upload, and object-URL lifecycle. Ingest/IN scope; for the export/OUT side (downloads, clipboard writes, generated filenames) see download-export-safety; for untrusted file content, SVG/HTML upload XSS, and size limits see frontend-security-baseline; for filename display and truncation see i18n-copy-and-layout."
---
...
5. **Treat accept and file.type as UX hints, never a type gate.** `accept` only filters the OS picker (MDN is explicit: it does not validate) and drag-drop/paste bypass it entirely. `file.type` is guessed from the extension, is spoofable, and is often an empty string. For a real check, sniff magic bytes client-side (`Blob.slice(0, N)` → `arrayBuffer`/`FileReader` → compare the signature) as a fast pre-filter, and re-validate on the server — that is the trust boundary. Header-only sniffing is still defeated by polyglots; route content-safety (SVG/HTML XSS, size / zip-bomb limits) to frontend-security-baseline.
...
- Use **frontend-security-baseline** for untrusted file content once it is in: SVG/HTML upload rendered as active markup (XSS), size / zip-bomb limits, and server-trust decisions — magic-byte sniffing on the client is a pre-filter, not the security boundary.
