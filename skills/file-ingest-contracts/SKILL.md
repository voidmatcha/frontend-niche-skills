...
3. **Re-set dropEffect on every dragover.** `dropEffect` is re-initialized from the user's requested action on every `dragenter`/`dragover`; set `dataTransfer.dropEffect = 'copy'` inside the `dragover` handler each time or the cursor won't reflect the operation. Use `dropEffect = 'none'` to visibly reject a spot.
4. **Read folders through items + webkitGetAsEntry, not files.** `DataTransfer.files` is a flat `FileList` with no directory support; only `DataTransfer.items` → `DataTransferItem.webkitGetAsEntry()` (prefixed; MDN says it may be renamed `getAsEntry()`, so code defensively for both) or the experimental, non-standard `getAsFileSystemHandle()` can recurse into a dropped directory — feature-detect (`'getAsFileSystemHandle' in DataTransferItem.prototype`) and fall back. `items` is only live inside the `drop`/`dragstart` handler, so capture entries synchronously before any `await`. For click-to-select folders use `<input webkitdirectory multiple>`; it selects only directories, not loose files, and each `File` carries a `webkitRelativePath`. The name is prefixed but MDN lists it as newly Baseline (checked 2026-09-25), so check older targets.
...
7. **One object URL per preview, and revoke it.** `URL.createObjectURL` mints a new blob URL every call and pins the blob in memory until `URL.revokeObjectURL`; re-creating one per render leaks. Revoke when the preview is replaced and on unmount (React: `useEffect` cleanup) — but not immediately after the image loads (for example in the `img` `onload` handler), which MDN says makes the image "unusable for user interactions (such as right-clicking to save the image or opening it in a new tab)". Object URLs are otherwise released automatically only when the document is unloaded, so long single-page sessions accumulate them.
...
- MDN, Drag operations — canceling `dragover` makes a drop target; `drop` must also be canceled; browsers diverge from the specification's `dragenter` cancellation step: <https://developer.mozilla.org/en-US/docs/Web/API/HTML_Drag_and_Drop_API/Drag_operations>
- MDN, DataTransfer.dropEffect — "For the dragenter and dragover events, dropEffect will be initialized based on what action the user is requesting"; set it per `dragover`; `none` rejects a spot: <https://developer.mozilla.org/en-US/docs/Web/API/DataTransfer/dropEffect>
...
- MDN, DataTransferItem.webkitGetAsEntry — directories via `items`; prefixed, "may be renamed to `getAsEntry()` in the future": <https://developer.mozilla.org/en-US/docs/Web/API/DataTransferItem/webkitGetAsEntry>
- MDN, `DataTransferItem.getAsFileSystemHandle()` — file/directory handles for dragged items; marked Experimental and Non-standard, feature-detect before use: <https://developer.mozilla.org/en-US/docs/Web/API/DataTransferItem/getAsFileSystemHandle>
- MDN, `HTMLInputElement.webkitdirectory` — "can only select directories instead of files", `webkitRelativePath` on each file; Baseline 2025 newly available (checked 2026-09-25): <https://developer.mozilla.org/en-US/docs/Web/API/HTMLInputElement/webkitdirectory>
- MDN, `Blob.type` (`File.type`) — "assumed based on the file extension", uncommon extensions "return an empty string", not a sole validation scheme: <https://developer.mozilla.org/en-US/docs/Web/API/Blob/type>
...
- MDN, URL.createObjectURL and URL.revokeObjectURL — the create/revoke APIs: <https://developer.mozilla.org/en-US/docs/Web/API/URL/createObjectURL_static>, <https://developer.mozilla.org/en-US/docs/Web/API/URL/revokeObjectURL_static>
- MDN, Using files from web applications — each `createObjectURL` call makes a unique URL that "must be released"; they are "released automatically when the document is unloaded"; revoking right after the image loads breaks right-click save and open-in-new-tab: <https://developer.mozilla.org/en-US/docs/Web/API/File_API/Using_files_from_web_applications>
